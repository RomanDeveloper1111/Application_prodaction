...
class AppCreateFormFirst(ModelForm):
    ...
    boss = forms.ModelChoiceField(queryset=Group.objects.filter(name='Коммерция директора')[0].user_set.all(),
                                  label='Директор')
    user_accountant = forms.ModelChoiceField(queryset=Group.objects.filter(name='Бухгалтерия')[0].user_set.all(),
                                             label='Бухгалтер')
